...
async function displaySalesStats() {
    ...
    // Получаем неподтвержденный (ожидающий) баланс со страницы продаж
    // ИСПРАВЛЕНИЕ: Всегда получаем именно status=paid (игнорирует фильтр периода — баланс в реальном времени)
            ...
            if (resp.ok) {
                ...
                if (result) {
                    ...
                    unconfEl.textContent = result.count > 0
                        ...
                        : '0 ' + sym;
                    // Стили: подсвечиваем, если есть ожидающие деньги
                    if (result.count > 0) {
...
function initializeSalesStatistics() {
    ...
    // Развернуть/свернуть «Показать ещё»
