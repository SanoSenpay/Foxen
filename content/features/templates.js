...
async function replaceTemplateVariables(template) {
    ...
    // 2.9: {orderlink} — ссылка на активный заказ в этом чате
...
async function addChatTemplateButtons() {
    ...
    const chatInput = document.querySelector('.chat-form-input .form-control');
    if (!chatInput) return;
    // Не показывать кнопки, если чат не выбран (состояние-заглушка)
    ...
    if (position === 'sidebar') {
        ...
        // Вставляем над оберткой chat-form, чтобы не нарушать внутреннюю flex-верстку формы
