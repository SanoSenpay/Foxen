// ── Надежный извлекатель JSON ─────────────────────────────────────────────────────
// Обрабатывает: ```json ... ```, лишний текст до/после, одинарные кавычки, запятые в конце
function robustExtractJSON(text) {
    if (!text) return null;

    // 1. Убираем markdown-блоки кода: ```json ... ``` или ``` ... ```
    text = text.replace(/^```(?:json)?\s*/im, '').replace(/\s*```\s*$/im, '').trim();

    // 2. Убираем лишний текст в начале и конце
    // Ищем первые [ или { и последние ] или }
    ...
    // 3. Исправляем частые ошибки ИИ в JSON:
    // - Лишние запятые перед ] или }
    candidate = candidate.replace(/,(\s*[}\]])/g, '$1');
    // - Одинарные кавычки → двойные кавычки (в простых случаях)
...
let _questions  = [];    // Сгенерированные ИИ вопросы
let _answers    = {};    // Ответы: { qIndex: answer }
let _currentQ   = 0;
...
async function auditLoadData() {
    ...
    // Получаем список лотов через фоновый скрипт
...
// ── Этап 2: Генерация вопросов (Запрос ИИ №1) ─────────────────────────────────
async function auditGenerateQuestions(data) {
    ...
    const totalLots   = realLots.length; // Ограничиваем 60 лотами, чтобы не перегружать контекст ИИ
    ...
    // Формируем условные секции — упоминаем только реальные проблемы, если они есть
    const titleIssues = [
...
// ── Этап 3: Генерация рекомендаций (Запрос ИИ №2) ───────────────────────────
...
function auditRenderQuestion(index) {
    ...
    // Сохраняем getValue для кнопки "Далее/Завершить"
...
function initializeAILotAudit() {
    ...
    const AUDIT_COOLDOWN_MS  = 30 * 24 * 60 * 60 * 1000; // Раз в 30 дней
    ...
    if (nextBtn) {
        nextBtn.addEventListener('click', async () => {
            ...
            if (_currentQ >= _questions.length - 1) {
                // Завершение — генерируем финальные рекомендации
