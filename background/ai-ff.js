...
// Основная функция для выполнения запроса к ИИ
async function makeAIRequest(finalPrompt) {
    ...
    try {
        ...
        if (result && result.response) {
            ...
            return { success: false, error: 'Неверный формат ответа ИИ или ответ пуст.' };
        }

    } catch (error) {
        console.error(`Сетевая ошибка при запросе к ИИ: ${error.message}`);
...
ГОТОВЫЙ ТЕКСТ ОТВЕТА:`;
        ...
        // Прокидываем промпт напрямую без изменений
        finalPrompt = textForAI;

    } else if (type === 'lot_audit') {
        // Для аудита лотов context содержит системный промпт, а textForAI — сообщение пользователя.
        ...
        if (jsonMatch) {
            try {
                ...
                return { success: true, data: cleanedJson };
                 ...
                 return { success: false, error: `ИИ вернул невалидный JSON даже после очистки: ${e2.message}` };
            }
        }
        return { success: false, error: `ИИ вернул невалидный JSON: ${e.message}` };
    }
}
    ...
    try {
        ...
        if (jsonMatch) {
            try {
                ...
                return { success: false, error: `ИИ вернул невалидный JSON при переводе (после очистки): ${e2.message}` };
            }
        }
        return { success: false, error: `ИИ вернул невалидный JSON при переводе: ${e.message}` };
    }
}
    ...
    try {
        ...
        if (jsonMatch) {
            try {
                ...
                return { success: false, error: `ИИ вернул невалидный JSON при генерации изображения (после очистки): ${e2.message}` };
            }
        }
        return { success: false, error: `ИИ вернул невалидный JSON при генерации изображения: ${e.message}` };
    }
}