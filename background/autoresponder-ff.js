...
function parseViaOffscreen(html, action) {
    // В Firefox DOMParser доступен прямо в фоне, offscreen не нужен
    return Promise.resolve(parseHtmlInBackground(html, action));
}

async function sendChatMessage(chatId, text, auth) {
    // Важно: для работы раннера FunPay требуются оба куки: golden_key и PHPSESSID.
    // Без PHPSESSID запрос вернет 200, но сообщение не будет отправлено.
    ...
    if (!res.ok) throw new Error(`Ошибка sendMessage, статус: ${res.status}`);
    const json = await res.json().catch(() => null);
    if (json?.error) throw new Error(`Ошибка раннера FunPay: ${json.error}`);
...
async function sendReviewReply(orderId, text, auth) {
    ...
    if (!res.ok) throw new Error(`Ошибка sendReviewReply, статус: ${res.status}`);
}
...
async function handleReview(msg, auth, settings) {
    ...
    try {
        ...
        if (!orderRes.ok) throw new Error(`Ошибка при получении страницы заказа: ${orderRes.status}`);
...
async function handleAutoDelivery(msg, auth, settings) {
    ...
    try {
        ...
        let deliveryText = '';
        let deliveryMode = 'secrets'; // По умолчанию используем секреты лота
        ...
        if (deliveryConfig?.mode === 'template' && deliveryConfig.text) {
            ...
            // Если конфига нет, но секреты есть — выдаем их
            deliveryText = secrets;
        }
...
async function runAutoResponderCycle() {
    ...
    try {
        ...
        if (!res.ok) throw new Error(`Ошибка Runner, статус: ${res.status}`);
