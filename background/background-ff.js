...
// --- Сбор статистики продаж (полный цикл без ограничений) ---
...
async function getAuthDetailsForBackground() {
    ...
    // Для работы с runner-ом FunPay теперь требует и PHPSESSID вместе с golden_key
...
// В Firefox DOMParser доступен прямо в фоне, так что offscreen-окно не нужно
...
async function runDiscordCheckCycle() {
    ...
    try {
        ...
        // Захватываем данные о просмотре товаров покупателями (для отображения в шапке чата)
...
async function processNextLotImport() {
    ...
    // Если процесса нет, он на паузе или всё уже импортировано — выходим
    ...
    try {
        ...
        // Особенности Firefox: lot.data может содержать массивы для полей с мульти-выбором.
        // URLSearchParams(obj) игнорирует массивы, поэтому собираем параметры вручную.
...
browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
    // В Firefox парсим прямо здесь, offscreen не требуется
    ...
    // RMTHUB PROXY (прокси для обхода CORS, так как контент-скрипты не могут делать кросс-доменные запросы)
    ...
    // --- Обработчики экспорта/импорта лотов ---
    ...
    // Сохранение или обновление одного лота (для массового редактора)
    if (request.action === 'saveSingleLot') {
        (async () => {
            try {
                ...
                // Особенности Firefox: обрабатываем возможные массивы для мульти-селектов
                const formData = new URLSearchParams();
    ...
    // Получение суммы неподтвержденного баланса
    ...
    if (request.action === 'startLotImport') {
        (async () => {
            const importProcess = {
                ...
                state: 'running', // Состояния: 'running', 'postponed'
    ...
    if (request.action === 'setGoldenKey') {
        (async () => {
            // Удаляем PHPSESSID, чтобы FunPay создал чистую сессию для нового golden_key.
            // Без этого FunPay может перепутать сессию и аккаунт, что приведет к логауту.
        ...
        (async () => {
            try {
                ...
                async function sfetch(url, opts = {}) {
                    // SSO: сначала идем через funpay.com/support/sso, чтобы получить куки сессии поддержки
                ...
                async function sfetchSupport(url, opts = {}) {
                    // Для support.funpay.com нужно сначала пройти SSO
                ...
                if (request.action === 'supportCloseTicket') {
                    ...
                    // Получаем токен со страницы тикета
                    const pageResp = await fetch(`${supportBase}/tickets/${ticketId}`, {
                        headers: { 'Cookie': baseCookie, 'User-Agent': ua }
                    });
                    const pageHtml = await pageResp.text();
                    // Ищем токен: сначала в input-е, затем в конфиге приложения
                    let token = null;
...
browser.alarms.onAlarm.addListener(async (alarm) => {
    ...
    if (alarm.name === 'fpToolsAutoRestore') {
        // Оповещаем все вкладки FunPay о необходимости проверить и восстановить лоты
        const tabs = await browser.tabs.query({ url: "https://funpay.com/*" });
        tabs.forEach(tab => {
...
function setupInitialAlarms() {
    browser.storage.local.get(['autoBumpEnabled', 'autoBumpCooldown', 'fpToolsDiscord', 'fpToolsAutoReplies'], (settings) => {
        ...
        // Периодическая проверка восстановления/отключения лотов (раз в 5 минут)
    ...
    if (changes.fpToolsAutoReplies) {
        ...
        browser.alarms.get(AUTO_RESPONDER_ALARM_NAME, (alarm) => {
            if (isEnabled && !alarm) {
                ...
                resetAutoResponderState(); // Очищаем сохраненный тег при выключении
            }
        });
    }
});
