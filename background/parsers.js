// background/parsers.js
// Функции парсинга HTML-страниц FunPay
...
function parseLotEditPage(html) {
    try {
        ...
        // Особенности Firefox: FormData(form) не подхватывает значения полей из документов, созданных через DOMParser.
        // (такой DOM инертен). Поэтому обходим все поля вручную.
        const dataObject = {};

        // <input> — все типы, кроме кнопок
        form.querySelectorAll('input').forEach(el => {
            if (!el.name) return;
            const type = (el.type || 'text').toLowerCase();
            if (['submit', 'button', 'image', 'reset'].includes(type)) return;
            if (type === 'checkbox' || type === 'radio') {
                // Берем только выбранные чекбоксы/радиокнопки
                if (el.checked) dataObject[el.name] = el.value;
            } else if (type === 'file') {
                // Пропускаем файлы (бинарные данные нельзя передать из фона)
            } else {
        ...
        // <select> — учитываем возможность множественного выбора
            ...
            if (el.multiple) {
                Array.from(el.selectedOptions).forEach(opt => {
                    ...
                    if (dataObject[el.name] === undefined) {
                        ...
                        // Сохраняем как массив — URLSearchParams создаст повторяющиеся ключи
        ...
        // Удаляем location — это специфичный для сессии параметр, он может помешать импорту на другом аккаунте
...
function parseUserCategories(html) {
    try {
        ...
        offerBlocks.forEach(block => {
            // Ссылка для управления (поднятие)
            const managementLink = block.querySelector('a.btn-plus');
            // Ссылка с названием для отображения
            ...
            if (managementLink && titleLink) {
                ...
                const lots = Array.from(lotItems).map(item => {
                    ...
                    // Отслеживаем автовыдачу для каждого лота отдельно
                ...
                // Категория считается категорией с автовыдачей, если хотя бы у одного лота она включена
...
function parseOrderPageForReview(html) {
    try {
        ...
        // Проверяем, не написан ли отзыв самим пользователем (пропускаем свои)
        // Используем более надежный селектор — ссылку на профиль в шапке заказа
        ...
        if (reviewAuthorId && currentUserId && reviewAuthorId === currentUserId) {
            return null; // Свой отзыв, пропускаем
        }
        ...
        // Название лота
        // Пробуем разные селекторы, которые FunPay использует для названия
        let lotName = '';
...
// Парсинг неподтвержденного (ожидаемого) баланса из шапки или страницы баланса
...
// Парсинг истории покупок клиента со страницы сделок
function parseBuyerHistory(html, buyerUsername) {
    // Если покупатель неизвестен — ничего не возвращаем (для безопасности)
    if (!buyerUsername) return [];
    try {
        const doc = new DOMParser().parseFromString(html, 'text/html');
        const orders = [];
        doc.querySelectorAll('a.tc-item').forEach(row => {
            // Сервер уже отфильтровал по имени пользователя — просто парсим все строки
...
// Парсинг страницы заказа для автовыдачи (секреты, ID лота, ID чата)
function parseOrderPageForDelivery(html) {
    try {
        const doc = new DOMParser().parseFromString(html, 'text/html');

        // Получаем содержимое товара из блока секретов
        ...
        // Краткое описание лота (для переменных в шаблонах)
        const shortDescHeader = Array.from(doc.querySelectorAll('.param-item h5'))
            .find(h => h.textContent.trim() === 'Краткое описание');
...
function parseSupportFields(html) {
    ...
    doc.querySelectorAll("input[name^='ticket[fields]'], select[name^='ticket[fields]'], textarea[name^='ticket[comment]']").forEach(el => {
        ...
        // radio: группируем все радиокнопки с одним именем
        if (inputType === 'radio') {
