    ...
    try {
        ...
        rows.forEach(row => {
            try {
                ...
                console.error("Foxen Offscreen: ошибка парсинга финоперации:", e);
            }
        });
        ...
        console.error("Foxen Offscreen: глобальная ошибка парсинга финансов:", e);
...
function parseSalesPage(html) {
    try {
        ...
        const orderRows = doc.querySelectorAll(".tc-item");
        ...
        orderRows.forEach(row => {
            try {
                ...
                console.error("Foxen Offscreen: Ошибка при парсинге одного заказа:", e, row);
            }
        });
        return { nextOrderId, orders };
    } catch (e) {
        console.error("Foxen Offscreen: Глобальная ошибка парсинга страницы продаж:", e);
        return { nextOrderId: null, orders: [] };
    }
}
...
function parseLotEditPage(html) {
    try {
        ...
        console.error("Foxen Offscreen: Error in parseLotEditPage", e);
        return null;
    }
}
...
function parsePublicLotForClone(html) {
    try {
        ...
        console.error("Foxen Offscreen: Error in parsePublicLotForClone", e);
        return null;
    }
}
...
function parseOfferEditPrice(html) {
    try {
        ...
        console.error("Foxen Offscreen: Error in parseOfferEditPrice", e);
        return null;
    }
}
...
function parseSellerLotPrice(html, offerId) {
    try {
        ...
        console.error("Foxen Offscreen: Error in parseSellerLotPrice", e);
        return null;
    }
}
...
function solveCloneForm(html, attributes, attributePairs) {
    try {
        ...
        console.error("Foxen Offscreen: Error in solveCloneForm", e);
        return null;
    }
}
...
function parseChatList(html) {
    try {
        ...
        console.error("Foxen Offscreen: Error parsing chat list.", e);
        return [];
    }
}
...
function parseUserLotsList(html) {
    try {
        ...
        console.error("Foxen Offscreen: Error in parseUserLotsList", e);
        return [];
    }
}
...
function parseUserCategories(html) {
    try {
        ...
        offerBlocks.forEach(block => {
            ...
            if (!titleLink) titleLink = block.querySelector('.offer-list-title a[href*="/lots/"], .offer-list-title a[href*="/chips/"]');
        ...
        console.error("Foxen Offscreen: Error in parseUserCategories", e);
        return [];
    }
}
// --- КОНЕЦ ИСПРАВЛЕНИЯ ---
...
function parseOrderPageForReview(html) {
    try {
        ...
        console.error("Foxen Offscreen: Error in parseOrderPageForReview", e);
        return null;
    }
}
...
function parseOrderPageForDelivery(html) {
    try {
        ...
        console.error('Foxen Offscreen: Error in parseOrderPageForDelivery', e);
        return null;
    }
}
